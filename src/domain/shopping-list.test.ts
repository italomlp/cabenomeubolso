...
describe('shopping list domain', () => {
  ...
  it('reuses retained actual price when purchase is toggled back on', () => {
    const list = createShoppingList({ items: [createItem({ actualUnitMinor: 2500, purchasedAt: null })] });
    const repurchased = markShoppingListItemPurchased(list, 'item-1', undefined, '2026-07-31T00:00:01.000Z');

    expect(repurchased.items[0]).toMatchObject({ actualUnitMinor: 2500, purchasedAt: '2026-07-31T00:00:01.000Z' });
  });
