...
export default function ShoppingScreen({ listId, dependencies, onSummary }: Props) {
  ...
  const purchase = async (itemId: string, text?: string) => { if (!runtime?.useCases.setItemPurchased || !list) return; try { const minor = text === undefined ? undefined : parseCurrencyMinor(locale, text, list.currencyCode); const next = await runtime.useCases.setItemPurchased(list.id, itemId, minor); setList(next); announceForAccessibility(t('shopping.purchasedAnnouncement')); } catch { setError(t('shopping.priceError')); } };
  const finalize = async (withConfirmation: boolean) => { if (!runtime || !list) return; try { const next = await runtime.useCases.finalizeList(list.id, { confirmUnpurchased: withConfirmation }); setList(next); setConfirm(false); onSummary?.(next.id); } catch { setConfirm(true); } };
  ...
  return <AppScreen contentStyle={{ paddingBottom: theme.space.xl }}>
    <AppColumn spacing={theme.space.md}>
      ...
      <BudgetRail actual={formatCurrencyMinor(locale, totals.actualMinor, list.currencyCode)} budget={formatCurrencyMinor(locale, list.budgetMinor, list.currencyCode)} remaining={formatCurrencyMinor(locale, Math.abs(totals.remainingMinor), list.currencyCode)} risk={risk} status={risk ? t('shopping.overBy') : t('shopping.remaining')} />
      {items.map((item) => { const bought = item.purchasedAt !== null; const unit = t(`units.${item.unitCode}`); return <AppColumn key={item.id} spacing={theme.space.xs} style={{ borderColor: theme.colors.border, borderWidth: 1, padding: theme.space.sm }}>
        <AppRow spacing={theme.space.sm}><AppColumn spacing={0}><AppText textStyle={{ fontWeight: '600' }}>{item.name}</AppText><AppText>{`${formatQuantityMilli(locale, item.unitCode, item.quantityMilli)} ${unit} · ${formatCurrencyMinor(locale, bought && item.actualUnitMinor !== null ? item.actualUnitMinor : item.plannedUnitMinor, list.currencyCode)} / ${unit}`}</AppText></AppColumn><AppButton label={bought ? t('shopping.undo') : t('shopping.buy')} accessibilitySelected={bought} onPress={() => bought ? runtime?.useCases.setItemUnpurchased?.(list.id, item.id).then(setList) : (setEditing(item.id), price.set(item.actualUnitMinor === null ? formatCurrencyMinor(locale, item.plannedUnitMinor, list.currencyCode) : formatCurrencyMinor(locale, item.actualUnitMinor, list.currencyCode)))} testID={`purchase-${item.id}`} variant={bought ? 'secondary' : 'primary'} /></AppRow>
      </AppColumn>; })}
