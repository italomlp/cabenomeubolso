...
export default function ShoppingScreen({ listId, dependencies, onSummary }: Props) {
  ...
  const purchase = async (itemId: string, text?: string): Promise<boolean> => {
    if (!runtime?.useCases.setItemPurchased || !list) return false;
    ...
    try {
      const next = await runtime.useCases.setItemPurchased(list.id, itemId, minor);
      setList(next);
  ...
  const finalize = async (withConfirmation: boolean) => {
    if (!runtime || !list) return;

    try {
      const next = await runtime.useCases.finalizeList(list.id, { confirmUnpurchased: withConfirmation });
      setList(next);
      setConfirm(false);
      setError(null);
      onSummary?.(next.id);
